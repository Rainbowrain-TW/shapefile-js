...
function shp (base, encoding, whiteList) {
  if (typeof base === 'string' && cache.has(base)) {
    return Promise.resolve(cache.get(base));
  }
  return shp.getShapefile(base, encoding, whiteList).then(function (resp) {
...
shp.parseZip = async function (buffer, encoding, whiteList) {
  let key;
  ...
  const geojson = names.map(function (name) {
    ...
    if (lastDotIdx > -1 && name.slice(lastDotIdx).indexOf('json') > -1) {
      ...
      if (zip[name + '.dbf']) {
        dbf = encoding ? parseDbf(zip[name + '.dbf'], encoding) : parseDbf(zip[name + '.dbf'], zip[name + '.cpg']);
      }
...
const handleDbf = async (base, encoding) => {
  ...
  return encoding ? parseDbf(dbf, encoding) : parseDbf(dbf, cpg);
};
const checkSuffix = (base, suffix) => {
  const url = new URL(base);
  return url.pathname.slice(-4).toLowerCase() === suffix;
};
shp.getShapefile = async function (base, encoding, whiteList) {
  if (typeof base !== 'string') {
    return shp.parseZip(base, encoding);
  }
  ...
  const results = await Promise.all([
    handleShp(base),
    handleDbf(base, encoding, whiteList)
  ]);
...
shp.parseDbf = function (dbf, cpg, encoding) {
  dbf = toBuffer(dbf);
  return encoding ? parseDbf(dbf, encoding) : parseDbf(dbf, cpg);
};
module.exports = shp;
